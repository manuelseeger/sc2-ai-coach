...
class Loader:
    ...
    if os.path.exists(os.path.join(ssim_dll_path, ssim_dll_name)):
        print_debug(
            "load '%s'" % (os.path.join(ssim_dll_path, ssim_dll_name)),
            textColor="golden",
        )
        dll = np.ctypeslib.load_library(ssim_dll_name, ssim_dll_path)
    else:
        print_debug(
            "load '%s' FAILED" % (os.path.join(ssim_dll_path, ssim_dll_name)),
            textColor="red",
        )
...
def SSIM(x, y, max_value=None, win_size=7):
    ...
    return skimage.measure.compare_ssim(
        x, y, win_size=win_size, data_range=max_value, multichannel=(x.ndim > 2)
    )


def SSIM_slow(
    x_image, y_image, max_value=255.0, win_size=7, use_sample_covariance=True
):
