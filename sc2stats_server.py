...
SEASON_START = datetime.date(2023, 4, 3)
...
def get_replays(today):
    ...
    list_of_files = [
        ...
        if today <= datetime.date.fromtimestamp(os.path.getmtime(f))
    ]
    ...
    for file_path in list_of_files:
        ...
        if (
            replay.category == "Ladder"
            and replay.region == "eu"
            and replay.real_type == "1v1"
        ):
            row = {
                "Map": replay.map_name,
    ...
    p3 = p2.rename(
        columns={"Player2_Result": "Player1_Result", "Player1_Result": "Player2_Result"}
    )
    p = p1.copy()
    ...
    print(
        p.to_string(
            sparsify=False, index_names=False, float_format=lambda f: str(int(f))
        )
    )
    return p.to_html(
        header=False,
        ...
        index_names=False,
        border=0,
        float_format=lambda f: str(int(f)),
    )


@app.route("/")
def get_stats():
    request_date = bottle.request.query.date or datetime.date.today().strftime(
        "%Y-%m-%d"
    )
